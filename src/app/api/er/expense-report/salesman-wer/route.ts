...
import { directusFetch, getVoucherStatusAndLinesCount, nowManila, toNumber } from "../utils";
  ...
  try {
    ...
    if (resource === "headers-list") {
      ...
      const latestVoucherForHeader = (headerId: number): VoucherItem | undefined => {
        ...
        if (!voucherIds || voucherIds.length === 0) return undefined;
        const latestVoucherId = Math.max(...voucherIds);
        return vouchersById[latestVoucherId];
      };
