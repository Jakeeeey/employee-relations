...
import { TaskBoard } from "@/modules/er/tasks/components/TaskBoard";
...
export default async function TasksPage() {
    ...
    if (token) {
        ...
        userId = (payload?.id || payload?.user_id || payload?.sub) as string | number | null;
    }
            ...
            <main className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
                {userId ? (
                    ...
                    <div className="flex h-full items-center justify-center text-muted-foreground">
                        <p>User not authenticated. Please log in to view tasks.</p>
                    </div>
                )}
            </main>
        </div>
