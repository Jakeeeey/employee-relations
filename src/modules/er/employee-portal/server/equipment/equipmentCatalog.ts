...
// source and as the shared `EquipmentCatalogItem` contract. It is NOT the
// runtime source: the live catalog is read from the
// `onboarding_equipment_item` collection through `./equipmentItemIo.ts`
// (`loadEquipmentCatalog()`), which maps DB rows onto this shape.
//
