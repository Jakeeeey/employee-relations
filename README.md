# Employee Relations (VOS - Master Workspace)

Modern and responsive Employee Relations management system built with Next.js 16, React 19, and Tailwind CSS 4. This project serves as a module within the VOS workspace, following the unified dependency stack and design patterns of the Master Template.
...
- **TypeScript (`^5`)**: Provides absolute type safety for complex HR flows, API responses, and shared UI types.
...
- **Lucide Icons (`^0.563.0`)**: A consistent icon set used for navigation, status indicators, and HR management.

### **State & Data Processing**
- **React Hook Form (`^7.71.1`)**: Manages high-performance data entry for complex forms like Employee Records or Multi-step Leave Management.
...
- **Recharts (`^2.15.4`)**: Powers workforce visualization dashboards, turnover trends, and performance metrics.
- **XLSX (`^0.18.5`)**: **[NEW]** Native Excel support for importing/exporting complex employee data sheets directly from the browser.

### **Advanced Utilities & Printing**
- **jsPDF (`^4.2.0`) & AutoTable (`^5.0.7`)**: Integrated to generate professional, pixel-perfect PDF reports and printable HR documents.
...
- **Browser Image Compression (`^2.0.2`)**: Optimizes file uploads like employee profile pictures or HR documents to ensure peak upload performance.
